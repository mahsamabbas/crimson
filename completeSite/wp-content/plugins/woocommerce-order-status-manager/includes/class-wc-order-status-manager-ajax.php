...
class WC_Order_Status_Manager_AJAX {
	...
	public function import_custom_order_statuses() {
		...
		wc_order_status_manager()->order_statuses->ensure_statuses_have_posts();
	...
	public function bulk_reassign_order_status() {
		...
		if ( isset( $_POST['old_status'] ) && isset( $_POST['new_status'] ) ) {
			...
			if ( ( $_POST['old_status'] != $_POST['new_status'] )
				...
				wc_order_status_manager()->order_statuses->handle_order_status_delete( $old_status->get_id(), $new_status->get_slug() );
