		...
		if ( ! empty( $next_statuses ) ) {
			...
			foreach ( $next_statuses as $next_status ) {
				...
				$custom_actions[ $next_status ] = array(
					'url'    => wp_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_mark_order_status&status=' . $next_status . '&order_id=' . $order->id ), 'woocommerce-mark-order-status' ),
		...
		if ( 'shop_order' == $post_type ) {
			...
			jQuery(function() {
				<?php foreach ( $custom_order_statuses as $status ) : ?>
					jQuery( '<option>' ).val( 'mark_<?php echo sanitize_html_class( $status->post_name ); ?>' ).text( '<?php printf( __( "Mark %s", 'woocommerce-order-status-manager' ), strtolower( get_the_title( $status->ID ) ) ); ?>' ).appendTo( "select[name='action']" );
					jQuery( '<option>' ).val( 'mark_<?php echo sanitize_html_class( $status->post_name ); ?>' ).text( '<?php printf( __( "Mark %s", 'woocommerce-order-status-manager' ), strtolower( get_the_title( $status->ID ) ) ); ?>' ).appendTo( "select[name='action2']" );
				<?php endforeach; ?>
			});
			</script>
			<?php
