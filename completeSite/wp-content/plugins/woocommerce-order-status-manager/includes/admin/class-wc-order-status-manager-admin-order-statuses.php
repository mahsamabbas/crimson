...
class WC_Order_Status_Manager_Admin_Order_Statuses {
		...
		<div id="order_status_options" class="panel woocommerce_options_panel">
			<div class="options_group">
				...
				woocommerce_wp_text_input( array(
					...
					'desc_tip'          => true,
					'description'       => __( 'Optional. If left blank, the slug will be automatically generated from the name. Maximum: 17 characters.', 'woocommerce-order-status-manager' ),
				) );
			...
			<div class="options_group">
				...
				<p class="form-field _icon_field">
					...
					<?php echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Optional status icon. If not supplied, then Name will be displayed to represent the status', 'woocommerce-order-status-manager' ) ); ?>
				</p>
				...
				<p class="form-field _action_icon_field">
					...
					<?php echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Optional action icon displayed in the action buttons for the next statuses.', 'woocommerce-order-status-manager' ) ); ?>
				</p>
			</div><!-- // .options_group -->
			...
			<div class="options_group">
				...
				<p class="form-field _next_statuses_field">
					...
					<?php echo SV_WC_Plugin_Compatibility::wc_help_tip( __( 'Zero or more statuses that would be considered next during normal order status flow. Action buttons will be available to move an order with this custom status to these next statuses.', 'woocommerce-order-status-manager' ) ); ?>
				</p>
				<?php
				...
				woocommerce_wp_checkbox( array(
					...
					'description' => __( 'Check this if this status implies a payment that has been received.', 'woocommerce-order-status-manager' ),
					'value'       => get_post_meta( $post->ID, '_is_paid', true ),
				) );
	...
	public function save_order_status_meta( $post_id ) {
		...
		update_post_meta( $post_id, '_is_paid',            isset( $_POST['_is_paid'] ) && $_POST['_is_paid'] ? 'yes' : 'no' );
	 ...
	 * @since 1.3.0
	 */
	function move_add_custom_statuses_action() {
		...
		if ( 'wc_order_status' === $typenow ) {
			...
			<div id="reassign-order-status-popup" style="display: none;">
				...
				<p class="singular" style="display: none">
					<a class="order-status-link"  href=""><?php /* translators: singular order marked with status name - %1$s: orders count (1), %2$s: order status name */
				...
				<p class="plural" style="display: none">
					<a class="order-status-link" href=""><?php /* translators: multiple orders marked with status name - %1$s orders count (n), %2$s: order status name */
